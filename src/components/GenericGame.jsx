...
const GenericGame = () => {
  ...
  const endGame = (result) => {
    ...
    setGameState(result);
    onOpen();
    markGameAsPlayed("Generic Game"); // Using context to mark the game as played
    setGameOutcome(result); // Using context to set the game outcome
  };
  ...
  return (
    <Flex direction="column" align="center" justify="center" minH="100vh" bg="gray.700" p={4}>
      ...
      <Flex justifyContent="space-between" w="full" mt={4}>
        <Button colorScheme="green" onClick={() => startGame()} isDisabled={gameState !== "ready"}>
          Play
        </Button>
        <Button colorScheme="red" onClick={() => endGame("won")} isDisabled={gameState !== "playing"}>
          Win
        </Button>
        <Button colorScheme="blue" onClick={() => endGame("lost")} isDisabled={gameState !== "playing"}>
          Lose
        </Button>
      </Flex>
