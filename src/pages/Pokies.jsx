...
const PokieCard = ({ pokie, played, isNew, isPopular, isPlaying, onPlay }) => {
  const { addGameToFavorites, markGameAsPlayed } = useContext(MockAPIContext);
  ...
  const handlePlay = () => {
    onPlay(pokie);
    addGameToFavorites(pokie.name); // Add the game to favorites when played
    markGameAsPlayed(pokie.name); // Mark the game as played using context
  };
  ...
  return (
    <Box maxW="sm" borderWidth="1px" borderRadius="lg" overflow="hidden" bg={bgColor} _hover={{ bg: hoverBgColor }}>
      ...
      <Box p="6" bg={bgColor} _hover={{ bg: hoverBgColor, boxShadow: boxShadow }} transition="background-color 0.3s, box-shadow 0.3s">
        ...
        <Button rightIcon={<FaPlay />} colorScheme="orange" variant="solid" onClick={onPlay} isLoading={isPlaying} isDisabled={played} boxShadow="0 0 12px 3px rgba(255, 165, 0, 0.8)">
          Play Now
        </Button>
      </Box>
    </Box>
...
const Pokies = () => {
  ...
  const [playedGames, setPlayedGames] = useState([]); // This should be retrieved from context or a backend
